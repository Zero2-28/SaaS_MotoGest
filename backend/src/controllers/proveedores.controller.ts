...
import { z } from "zod";
import { createProveedorSchema, updateProveedorSchema } from "../schemas/schemas";
import { toAppError } from "../lib/appError";
import {
  createProveedorService,
  deleteProveedorService,
  getProveedorByIdService,
  listProveedoresService,
...
export const listProveedores = async (_req: Request, res: Response): Promise<void> => {
  try {
    const items = await listProveedoresService();
...
export const getProveedorById = async (
  ...
  try {
    ...
    const item = await getProveedorByIdService(Number(validation.data.id));
...
export const createProveedor = async (
  ...
  try {
    const validation = createProveedorSchema.safeParse(req.body);
    if (!validation.success) {
      sendError(res, `Validación fallida: ${validation.error.errors[0].message}`);
      return;
    }
    const item = await createProveedorService(validation.data);
...
export const updateProveedor = async (
  ...
  try {
    ...
    const bodyValidation = updateProveedorSchema.safeParse(req.body);
    if (!bodyValidation.success) {
      sendError(res, `Validación fallida: ${bodyValidation.error.errors[0].message}`);
      return;
    }
    const item = await updateProveedorService({
...
export const deleteProveedor = async (
  ...
  try {
    ...
    const item = await deleteProveedorService(Number(validation.data.id));
    sendSuccess(res, item);
  } catch (error: unknown) {
    const appError = toAppError(error);
    sendError(res, appError.message, appError.statusCode);
