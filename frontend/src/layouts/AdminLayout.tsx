...
> = {
  general:         { icon: Info,          colorClass: 'text-info',   label: 'General'         },
  stock_bajo:      { icon: AlertTriangle, colorClass: 'text-warning', label: 'Stock bajo'      },
  pedido_nuevo:    { icon: ShoppingBag,   colorClass: 'text-brand',  label: 'Pedido nuevo'    },
  pago_confirmado: { icon: CreditCard,    colorClass: 'text-success',  label: 'Pago confirmado' },
}
  ...
  return (
    <li
      className={cn(
        ...
        notif.leida ? 'opacity-50' : 'bg-chrome-50'
      )}
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-ink leading-snug">{notif.titulo}</p>
        <p className="text-xs text-chrome-600 leading-snug mt-0.5 line-clamp-2">
          {notif.mensaje}
        </p>
      </div>
      {!notif.leida && (
        <button
          className="mt-0.5 shrink-0 h-2 w-2 rounded-full bg-brand hover:bg-brand-900 transition-colors"
...
function NotificacionesPanel({
  ...
  return (
    <div
      ...
      className="absolute right-0 top-full mt-2 z-50 w-80 rounded-lg border border-chrome-200 bg-white shadow-card-lg animate-fade-in"
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-chrome-200">
        <div className="flex items-center gap-2">
          <Bell className="h-4 w-4 text-chrome-600" aria-hidden />
        ...
        {noLeidas > 0 && (
          <button
            className="text-xs text-chrome-600 hover:text-brand transition-colors"
      ...
      <ScrollArea className="max-h-80">
        {notifs.length === 0 ? (
          <div className="py-10 text-center">
            <Bell className="h-8 w-8 mx-auto text-chrome-300 mb-2" />
            <p className="text-xs text-chrome-600">Sin notificaciones pendientes</p>
          </div>
        ) : (
...
const ROL_BADGE: Record<RolEmpleado, string> = {
  ...
  vendedor:   'bg-turbo text-white',
  repartidor: 'bg-info text-white',
}
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="bg-white text-ink">
        <DialogHeader>
          ...
          <DialogDescription className="text-chrome-600">Información de tu cuenta y cambio de contraseña.</DialogDescription>
        </DialogHeader>
          ...
          <div className="flex flex-col items-center gap-3 py-2">
            <div className="relative">
              ...
              <button
                ...
                className="absolute bottom-0 right-0 flex h-6 w-6 items-center justify-center rounded-full bg-white border border-chrome-300 shadow-card hover:bg-chrome-50 transition-colors"
                aria-label="Cambiar foto de perfil"
              >
                <Camera className="h-3.5 w-3.5 text-chrome-700" />
              </button>
              <input
                ref={fileInputRef}
                type="file"
            ...
            <div className="text-center space-y-0.5">
              <p className="font-semibold text-ink">{empleado.nombre}</p>
              <p className="text-xs text-chrome-600">{empleado.email}</p>
            </div>
            <span className={cn('rounded-full px-3 py-0.5 text-xs font-semibold capitalize', ROL_BADGE[empleado.rol])}>
              {empleado.rol}
            </span>
            {empleado.sucursalId && (
              <p className="text-xs text-chrome-600">Sucursal #{empleado.sucursalId}</p>
            )}
          </div>
        )}
        ...
        <p className="text-xs font-medium text-chrome-700 uppercase tracking-wider flex items-center gap-1">
          <KeyRound className="h-3 w-3" />Cambiar contraseña
        </p>

        {exito && (
          <div role="status" className="rounded-md border border-success-100 bg-success-50 p-2.5 text-xs text-success-700">
            Contraseña actualizada correctamente.
          </div>
        )}
        {errorMsg && (
          <div role="alert" className="rounded-md border border-danger-100 bg-danger-50 p-2.5 text-xs text-danger-600">
            {errorMsg}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="grid gap-3">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="pwd-actual">Contraseña actual <span className="text-brand" aria-hidden>*</span></Label>
            <div className="relative">
              <Input
                id="pwd-actual"
                type={mostrarActual ? 'text' : 'password'}
                className="pr-9 bg-mist border-chrome-200 text-ink placeholder:text-chrome-400"
                {...register('passwordActual')}
                aria-invalid={!!errors.passwordActual}
              />
              <button
                type="button"
                className="absolute right-3 top-1/2 -translate-y-1/2 text-chrome-400 hover:text-chrome-700"
            ...
            {errors.passwordActual && (
              <p role="alert" className="text-xs text-danger">{errors.passwordActual.message}</p>
            )}
          </div>

          <div className="flex flex-col gap-1.5">
            <Label htmlFor="pwd-nuevo">Nueva contraseña <span className="text-brand" aria-hidden>*</span></Label>
            <div className="relative">
              <Input
                id="pwd-nuevo"
            ...
            {errors.passwordNuevo && (
              <p role="alert" className="text-xs text-danger">{errors.passwordNuevo.message}</p>
            )}
          </div>

          <div className="flex flex-col gap-1.5">
            <Label htmlFor="pwd-confirm">Confirmar contraseña <span className="text-brand" aria-hidden>*</span></Label>
            <Input
              id="pwd-confirm"
              type="password"
            ...
            {errors.confirmarPassword && (
              <p role="alert" className="text-xs text-danger">{errors.confirmarPassword.message}</p>
            )}
          </div>

          <DialogFooter>
            <Button type="button" variant="secondary" onClick={handleClose} className="bg-white border border-chrome-200 text-chrome-700 hover:bg-chrome-50">Cerrar</Button>
...
function AdminSidebar({
  ...
  return (
    <aside
      className={cn(
        'flex flex-col border-r border-white/5 bg-metal shadow-metal-lg transition-all duration-200',
        collapsed ? 'w-16' : 'w-64'
      )}
      aria-label="Navegación de administración"
    >
      {/* Logo + colapso */}
      <div className="flex h-16 shrink-0 items-center justify-between border-b border-white/10 bg-brand-950 px-3">
        {!collapsed && (
          <div className="flex items-center overflow-hidden pl-1">
            <img
              src={assets.logo}
              alt="RE MOTOS — Panel de administración"
        ...
        {collapsed && (
          /**
           * Colapsado: se recorta el lockup para dejar solo el isologo.
           * El engranaje está en (160,125) del viewBox 850x300; con el SVG a
           * 48px de alto queda centrado en la caja de 40px desplazándolo -5.6px.
           */
          <div className="mx-auto h-10 w-10 overflow-hidden rounded-md">
            <img
              src={assets.logo}
              alt="RE MOTOS"
              className="max-w-none"
              style={{ height: 48, width: 48 * LOGO_RATIO, marginLeft: -5.6 }}
            />
          </div>
        )}
        {!collapsed && (
      ...
      {collapsed && (
        <button
          ...
          className="mx-auto mt-2 rounded-md p-1.5 text-brand-300 transition-colors hover:bg-white/10 hover:text-white"
        >
      ...
      <ScrollArea className="flex-1 py-4">
        <nav className="flex flex-col gap-0.5 px-2">
          <p className="px-3 pb-2 text-[9px] font-semibold uppercase tracking-widest text-brand-300/70">
            MAIN MENU
          </p>
          {NAV_ITEMS.filter(({ roles }) => !empleado?.rol || roles.includes(empleado.rol)).map(({ to, icon: Icon, label, exact }) => (
            <div key={to} className="relative group">
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive
                      ? 'bg-white/[0.12] text-white shadow-metal before:absolute before:inset-y-1.5 before:left-0 before:w-0.5 before:rounded-full before:bg-brand-300'
                      : 'text-brand-200 hover:bg-white/[0.08] hover:text-white'
                  )
                }
              >
              ...
              {collapsed && (
                <span className="pointer-events-none absolute left-full top-1/2 z-50 ml-2 -translate-y-1/2 whitespace-nowrap rounded-md border border-white/10 bg-brand-950 px-2 py-1 text-xs text-white opacity-0 shadow-metal transition-opacity group-hover:opacity-100">
                  {label}
                </span>
              )}
            </div>
      ...
      <div className="shrink-0 border-t border-white/10 p-3">
        {!collapsed && empleado && (
          <button
            className="mb-3 w-full rounded-md border border-white/5 bg-white/5 p-2 text-left transition-colors hover:bg-white/10"
            onClick={onPerfil}
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="truncate text-xs font-medium text-white">{empleado.nombre}</p>
                <p className="text-xs capitalize text-brand-300">{empleado.rol}</p>
              </div>
            </div>
          </button>
        )}
        {collapsed && (
          <button
            className="mb-2 flex w-full items-center justify-center rounded-md p-1 transition-colors hover:bg-white/10"
            onClick={onPerfil}
            aria-label="Abrir perfil"
          >
            <Avatar
        ...
        <button
          className={cn(
            'flex w-full items-center rounded-md px-3 py-2 text-sm font-medium text-brand-200 transition-colors hover:bg-danger/10 hover:text-danger-50',
...
function AdminTopbar() {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-chrome-200 bg-white px-6 shadow-card">
      <div />
      <div className="relative flex items-center gap-2">
        <button
          className="relative p-2 rounded-md text-chrome-700 hover:bg-chrome-50 transition-colors"
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-mist">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <AdminTopbar />
        <main className="flex-1 overflow-y-auto p-6 bg-mist">
          <Outlet />
        </main>
      </div>
